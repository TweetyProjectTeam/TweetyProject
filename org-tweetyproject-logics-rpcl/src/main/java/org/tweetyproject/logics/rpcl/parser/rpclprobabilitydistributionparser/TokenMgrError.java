 ...
 * <p>
 * This method constructs a descriptive error message based on the location in the input
 * where the error occurred, the character that caused the error, and the text that was parsed
 * immediately before the error. If the end-of-file (EOF) was encountered, this is indicated
 * in the message.
 * </p>
 *
...
* including whether the end-of-file (EOF) was encountered, the current lexical state, the position
* in the input (line and column), the text immediately following the error, and the character that
* caused the error. It also includes a reason code for further categorization of the error.
* </p>
*
* @param EOFSeen A boolean indicating whether the end-of-file (EOF) was encountered during the error.
* @param lexState The lexical state of the parser when the error occurred.

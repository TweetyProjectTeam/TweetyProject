 ...
 *
 * to implement this class, you need to define a selection, a termination and a condition function
 * Selection function a(UA, UC, C):     selects and returns a subset of the initial sets
 * Acceptance function b((AF, S)):      If the given state satisfies a specific condition, its extension may be accepted by the associated serialised semantics
 *
 * @author Lars Bengel
 */
public abstract class SerialisableExtensionReasoner extends AbstractExtensionReasoner {
     ...
     */
    public abstract boolean acceptanceFunction(TransitionState state);
    ...
    protected Collection<Extension<DungTheory>> getModelsRecursive(TransitionState state, Collection<Extension<DungTheory>> result) {
        ...
        if (this.acceptanceFunction(state)) {
    ...
    public static SerialisableExtensionReasoner getSerialisableReasonerForSemantics(Semantics semantics){
        switch(semantics){
            case CO: return new SerialisedCompleteReasoner();
            case GR: return new SerialisedGroundedReasoner();
            case PR: return new SerialisedPreferredReasoner();
            case ST: return new SerialisedStableReasoner();
            case ADM: return new SerialisedAdmissibleReasoner();

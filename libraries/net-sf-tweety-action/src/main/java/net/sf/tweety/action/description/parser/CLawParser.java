...
public class CLawParser extends Parser<CActionDescription, Formula> {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sf.tweety.commons.Parser#parseFormula(java.lang.String)
	 */
	@Override
	public Formula parseFormula(String s) throws ParserException, IOException {
		...
		if (s.contains(" requires ")) {
			...
			if (!wrongKey.isEmpty())
				throw new ParserException("Unexpected keyword(s) '" + wrongKey.toString() + "' in formula '" + s
						+ "' due to wrong order or multiple use.");
	...
	private Formula parseCausedFormula(String s, String reqString) throws ParserException, IOException {
		...
		if (!wrongKey.isEmpty())
			throw new ParserException("Unexpected keyword(s) '" + wrongKey.toString() + "' in formula '" + s
					+ "' due to wrong order or multiple use of keywords.");
	...
	protected Set<String> containedKeywords(String s) {
		final String[] keywords = { "caused ", " if ", " after ", " requires ", "inertial ", "default ", " causes ",
				"always ", "nonexecutable ", " may cause " };
